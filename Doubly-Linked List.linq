...
void Main()
{
	...
	linkedList.Remove(5);
	linkedList.PrintList();
	linkedList.Tail.Next.Dump();
}

// Main difference in doubly-linked list is now maintaining 'previous' reference.  Biggest performance benefit is when removing from the end of the list.
public class MyLinkedList<T> : ICollection<T>
{
	...
	public void RemoveLast()
	{
		if (this.Count != 0)
		{
			...
			{
				// Removal for doubly-linked list much easier.  This is where biggest performance improvement is over singly-linked list.
	...
	public bool Remove(T item)
	{
		...
		else
		{
			...
			// Tail moved out of iteration.
			if (this.Tail.Value.Equals(item))
			{
				this.Tail = this.Tail.Previous;
				this.Tail.Next = null;
			}
